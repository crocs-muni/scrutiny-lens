...
A public, static website. No app server, no login — no first-party telemetry, analytics, or beacons (the static host unavoidably sees page loads). Deployed on GitHub Pages; any static host works.
...
**Share:** a button copies `${origin}${mount}/event/${nevent}` — `${mount}` is the deployment's base path (`/scrutiny-lens/` on this cycle's Pages project site, per §8; `/` at a domain root) — where `nevent` is a NIP-19 bech32 `nevent1…` encoding the root event id **plus relay hints** (up to 3 relays where the sharer observed the event, plus author and kind TLVs) — a standard, interoperable nostr blob (nostr-tools `nip19.neventEncode`/`decode`). One opaque blob, no query params; `/event/<nevent>` routing is our own convention (njump-style), not a nostr standard. The same share action offers a second copy channel: the bare NIP-21 `nostr:<nevent>` string for native clients. Same shape later serves the SEO idea (`/event/nevent1…` pages). Opening it runs the full fetch for that root — local cache first, then hinted relays, then the author's NIP-65 write relays, then the configured pool: the event, its patch chain (`#e`+`#t`), bindings, and kind-5 deletions (`deletionsFor`, polled — relays may omit them). All states (incl. retracted) are discovered by normal resolution. A recipient with different relays/trust sees the same canonical bytes but a possibly different overlay view; hinted relays that failed are named to them (§4).
...
- New components: SidebarRecents (extend SidebarNav: query, timestamp, unseen dot, close), ResultCard (seed: RecommendationCard), FacetGroup (checkbox+count rows; seed: SearchList+ToolChips), ChatMessage + SourceList (citation pills), CitationMark (colored underline from GlideHighlight) **plus a citation-coordination store** linking pill ↔ prose span ↔ graph-node ring, DetailDrawer (bottom drawer below the canvas: sticky counted sections Summary/Content/History/Files, resizable splitter, 32px collapsed handle, ring persistence), Timeline (patch history entries), EmptyState, KeyField, RelayDot.
- Result card anatomy (locked by the design board): title (AI, unmarked), snippet, identifier chips 0..n mono, warning-only status pill (shown only for retracted/provisional), icon+word counts (metadata / files / updates), publisher profile chip (kind-0 avatar+name, identicon fallback, fades in on fetch), whole-card click target with hover ring (no action buttons on faces).
- Graph node anatomy (locked by the design board): every node carries a kind-mapped icon (Tabler icons; neutral tile, tinted stroke only when warn), a concise title, publisher avatar+name (npub prefix in mono as fallback), relative time, and `edited ×N` only when patches exist (neutral, ink-2 mono). The subject adds a one-line description + icon-number counts; a linked record omits both. Dense mode = rendered-pixel floors (full card → icon+mono line → icon disc); selection/hover forces full detail at any zoom.
- **Subject-graph glossary** (issue #29 canon): **subject graph** = the nodes gathered around one selected subject; **subject** = the centered event (the canvas anchor) — normally a product; a linked record can also be centered when opened as the dossier subject; **linked record** = a metadata record bound to the subject; **related product** = another product sharing a linked record with the subject, shown collapsed with a +N badge until expanded.
- Drawer Files row anatomy (issue #77): artifact family icon (mime/extension bucket) · label in mono (descriptor label, else URL middle-clip) · size / SHA-256 (mid-clip; full on hover) · open↗ external (noopener) · verb chip + counterparty deep-link. Footer icon pair, identical across GraphNode and ResultCard: IconFile = bound records (metadata), IconLink = artifacts (pdf/csv/…).
...
- Writing rule: **monospace = machine-made/verified (ids, tags, hashes, quotes); sans = AI-written prose.** Exception (J2 board anatomy, ruling 2026-09-04): facet value labels render sans 14px; card count rows and their numerals stay mono.
