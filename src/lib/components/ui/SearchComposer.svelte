...
<div data-search-composer class="relative">
	{#if modelOpen}
		<!-- model menu: canon's floating listbox above the controls row,
			glide rows reskinned with the shared GlideHighlight. -->
		<div
			role="listbox"
			tabindex="-1"
			class="absolute bottom-full left-0 z-10 mb-2 w-64 max-w-full rounded-card bg-surface p-1 shadow-raised"
				...
				>
					{#each models as id (id)}
						<button
							...
							class="relative z-10 flex h-7.5 w-full items-center gap-2 rounded-chip px-2 text-left"
						>
							<span class="min-w-0 flex-1 truncate font-mono text-[12.5px] text-ink">{id}</span>
	...
	>
		...
		<div class="grid grid-cols-[auto_minmax(0,1fr)_28px] items-end gap-x-1">
			...
			<button
				...
				class="col-start-1 row-start-1 flex h-7 shrink-0 items-center gap-1 rounded-control px-1.5 font-mono text-[12px] font-medium text-ink-2 transition-colors duration-150 hover:bg-hover hover:text-ink {modelOpen
					? 'bg-hover text-ink'
					: ''}"
			>
