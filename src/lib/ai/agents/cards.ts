...
import { z } from "zod";
import { type AIResult, type AIKind, type CallLLM } from "../output";
import { generateRecords } from "../records";
import { writeDeadLetter } from "../deadLetter";
import { projectSkeleton, tagValues, type SkeletonStats } from "../projector";
...
/**
 * Lenient per-record draft (KV, issue #52) — every field optional; strict
 * per-item validation happens in finalizeCard via CardVMSchema.safeParse.
 * `entityId` binds the record back to its graph (a salvaged partial page
 * must not shift drafts onto the wrong cards). The nested snippet is flat
 * dotted keys — `snippet.text` and `snippet.highlights.N.start/.len`.
 */
const CardDraft = z
  .object({
    entityId: z.string().min(1),
...
/** Keys the model may emit — the KV classifier's known set (issue #52). The
 * highlight index slots (0..2) cover the documented ≤3 highlights; a model
 * emitting more simply has the extra blocks ignored by the longest-prefix
 * classifier. */
const CARD_KEYS = [
  "entityId",
  "title",
  ...
  "snippet.highlights.1.start",
  "snippet.highlights.1.len",
  "snippet.highlights.2.start",
  "snippet.highlights.2.len",
] as const;
...
function deriveStatus(event: NostrEvent): CardStatus {
  const ctx = [...tagValues(event, "status"), ...tagValues(event, "lifecycle")]
    .join(" ")
    .toLowerCase();
...
export function metaSegmentsRule(fields: {
  ...
  if (fields.scheme && fields.scheme.trim().length > 0)
    segments.push(fields.scheme.trim());
  if (fields.assurance && EAL.test(fields.assurance))
    segments.push(fields.assurance);
...
export function matchBandRule(
  ...
  const facetHits = facetValues.filter((v) =>
    terms.some((t) => normalize(v).includes(t)),
  );
...
function rebinds(reason: string, input: DeterministicInput): boolean {
  ...
  if (idm) {
    ...
    return input.identifiers.some(
      (i) => normalize(i).includes(val) || val.includes(normalize(i)),
    );
  }
  ...
  if (stm) {
    ...
    return (
      label === input.status.toLowerCase() ||
      label === val ||
      val === input.status.toLowerCase()
    );
  }
  ...
  if (fm) {
    ...
    for (const key of Object.keys(input.facets)) {
      ...
      if (
        vals.some(
          (v) => normalize(v).includes(val) || val.includes(normalize(v)),
        )
      )
        return true;
    }
...
export function snippetRules(
  ...
  if (terms.length > 0 && terms.some((t) => !norm.includes(t)))
    return bad("R1");
  ...
  if (reference.length > 0 && bigramSimilarity(text, reference) > 0.85)
    return bad("R3");
...
function buildDeterministic(graph: CardGraph) {
  ...
  const assurance =
    tagValues(event, "eal").find((v) => EAL.test(v)) ?? undefined;
  const statusLabel_ = statusLabel(status);
  const metaSegments = metaSegmentsRule({
    scheme,
    assurance,
    status: statusLabel_,
  });
...
function finalizeCard(
  ...
  const det = buildDeterministic(graph);
  const candidates =
    det.identifiers.length > 0 ? det.identifiers : [bestFallbackId(graph)];
  // Match reasons — invented ones are dropped + dead-lettered.
  const reasonInput: DeterministicInput = {
    ...ctx,
    identifiers: candidates,
    status: det.status,
    facets: det.facets,
  };
  const matchReasons = validateMatchReasons(
    draft?.matchReasons ?? [],
    reasonInput,
  );
  ...
  if (
    draft?.snippet?.text !== undefined &&
    draft.snippet.text.trim().length > 0
  ) {
    ...
    if (sr.ok && sr.snippet) {
      ...
      writeDeadLetter({
        ...
        model: ctx.model,
        payload: {
          text: draft.snippet.text,
          highlights: draft.snippet.highlights,
        },
  ...
  const candidate = {
    ...
    title:
      typeof draft?.title === "string" && draft.title.trim().length > 0
        ...
        : "",
    vendor:
      typeof draft?.vendor === "string" && draft.vendor.trim().length > 0
        ? draft.vendor
        : undefined,
    typeToken:
      typeof draft?.typeToken === "string" && draft.typeToken.length > 0
        ? draft.typeToken
        : "unknown",
    category:
    ...
    identifiers: candidates,
    scheme:
      typeof draft?.scheme === "string" && draft.scheme.trim().length > 0
        ? draft.scheme
        : det.scheme,
  ...
  for (let i = 0; i < graphs.length; i += BATCH_SIZE) {
    ...
    const res = await generateRecords({
      ...
      messages: [
        {
          role: "user",
          content: `Interpret each graph into one summary card, echoing its \`entityId\` EXACTLY.\n${payload}\n\nEmit one record per graph. Highlights are optional index keys (snippet.highlights.0.start / .len, at most 3).`,
        },
      ],
    ...
    for (let j = 0; j < page.length; j++) {
      ...
      cards.push(
        finalizeCard(page[j], query, drafts.get(page[j].entityId), ctx),
      );
    }
  }
