...
import {
	...
	snippetRules
} from '$lib/server/ai/agents/cards';
...
beforeEach(() => {
	dir = mkdtempSync(join(tmpdir(), 'scrutiny-cards-'));
...
function deadLetterRows() {
	return db
		.prepare('SELECT entityId, reason FROM dead_letter')
		.all() as Array<{ entityId: string; reason: string }>;
}
