...
describe("parseRecords", () => {
  ...
  it("splits blocks on one OR MANY blank lines (tolerant)", () => {
    ...
    expect(parseRecords(text, { knownKeys: KNOWN }).records).toHaveLength(2);
  });
  ...
  it("a non-key line continues the previous key (trailing-space joined)", () => {
    const text =
      "id: a\n" + "title: First line\n" + "  continued line\n" + "snippet: S";
    const { records } = parseRecords(text, { knownKeys: KNOWN });
  ...
  it("returns an empty list when nothing survives", () => {
    const { records, issues } = parseRecords(
      "complete gibberish\nno keys here",
  ...
  it("numbers: an integer string coerces, a non-numeric value drops the key", () => {
    ...
    const { records: bad } = parseRecords(
      "id: a\ntitle: T\nsnippet: S\nmatch: high",
  ...
  it("floats: a decimal coerces, a percentage coerces, a non-numeric value drops the key", () => {
    const ok = parseRecords("id: a\ntitle: T\nsnippet: S\nmatch: 0.65", {
      knownKeys: KNOWN,
      floats: ["match"],
    });
    expect(ok.records[0].match).toBe(0.65);
    const pct = parseRecords("id: a\ntitle: T\nsnippet: S\nmatch: 75%", {
      knownKeys: KNOWN,
      floats: ["match"],
    });
    expect(pct.records[0].match).toBe(75);
    const bad = parseRecords("id: a\ntitle: T\nsnippet: S\nmatch: high", {
      knownKeys: KNOWN,
      floats: ["match"],
    });
  ...
  it("booleans: true/false coerce, anything else drops the key", () => {
    const t = parseRecords("id: a\ntitle: T\nsnippet: S\nmatch: true", {
      knownKeys: KNOWN,
      booleans: ["match"],
    });
    expect(t.records[0].match).toBe(true);
    const f = parseRecords("id: a\ntitle: T\nsnippet: S\nmatch: false", {
      knownKeys: KNOWN,
      booleans: ["match"],
    });
  ...
  it("a single JSON object (not an array) is NOT salvaged as one record", () => {
    ...
    expect(
      parseRecords(text, { knownKeys: KNOWN, schema: Rec }).records,
    ).toEqual([]);
  });
